...
measurement_noise_intensity = 0.01
x_index = 0
y_index = 2
use_assimilation = 1
...
measurement_noise_limit_agent = 1000.
noise_factor_agent_coeff = .1
num_of_agents = 4
agent_positions = np.array([[0 , 0] ,[5, 2] , [15 , 4] , [2000 , 1000]])
...
sigma_x_init = 1
sigma_y_init = 1
initial_state = array([[1 + randn()*sigma_x_init, 0 ,1 + randn() * sigma_y_init , 0]] , dtype='float64').T  # Initial state [x, y]
P = eye(state_dim , dtype='float64') * 100.0  # Initial estimation error covariance - P
Q = array([[0. , .1 , 0. , .1]]).T *process_noise_intensity @ array([[0. , .1 , 0. , .1]])*dt  # Q Process noise covariance
# Q = velocity_model_noise(var=process_noise_intensity ,dt=dt)
...
agents_mc = []
number_of_runs = 2
process_noise_intensity_arr = np.linspace(1,2,number_of_runs)
for run in range(number_of_runs):
    process_noise_intensity =  process_noise_intensity_arr[run]
    Q = array([[0., .1, 0., .1]]).T * process_noise_intensity @ array([[0., .1, 0., .1]]) * dt  # Q Process noise covariance
    # initialize KF
    for agent in agents:
    ...
    for measurement_index in range(len(agent.measurements)):
        for agent in agents:
            ...
            #update
            agent.filter.update_EKF(measurement = agent.measurements[measurement_index], R = agent.filter.R + agent.R_arr[measurement_index])  # feeding the update with measurement cov*distance factor
