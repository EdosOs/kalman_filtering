...
def input(inp_type , t , amplitude , pulse_time = 0 , dt = 2 , delay = 0,t_transition = [0]):
    amp = [0]
    for index in range(len(t_transition)-1):
        if t>t_transition[-1]:
            amp[0] = amplitude[-1]
        if t<t_transition[0]:
            amp[0] = amplitude[0]
        if t<t_transition[index+1] and t_transition[index]<t:
