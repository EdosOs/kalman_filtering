...
def acceleration_model(t_start,t_stop,model_noise_var , dt , B , G , input_type = 'sin',input_amplitude = 1):
    t_span = [t_start, t_stop]
    ode_fcn = lambda T,X: [X[1]+X[2]*T,X[2] , 0] + squeeze(B , axis=1) * input(input_type , T , input_amplitude) + squeeze(G , axis=1) * randn() * model_noise_var
