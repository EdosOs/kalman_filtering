...
for agent in agents:
    ...
    plt.title(f'agent {agent.id} at position ({agent.position[0,0]},{agent.position[0,1]}) Y state estimation(updated) and measurements')
    plt.xlabel('time')
    plt.ylabel('amplitude')
    plt.show()
...
for agent in agents:
    ...
    ax.plot()
    plt.legend(['X estimation' ,  'X real'])
    plt.title(f'agent {agent.id} at ({agent.position[0,0]},{agent.position[0,1]}) XY state estimation (updated) and measurements')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.show()
    ...
    for agent in agents:
        ...
        ax.plot(T, X[0], Y[0], '--r')
        for agent in agents:
            ax.scatter(T[0], agent.position[0,0], agent.position[0,1], 'green')
